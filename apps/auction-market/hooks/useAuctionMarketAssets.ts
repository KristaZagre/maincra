...
import { LiquidityPositionRepresentation, TokenRepresentation } from 'features/context/representations'
  ...
  const tokens = useMemo(
    () =>
      tokenRepresentations
        ?.filter((token) => token.id.toLowerCase() !== bidTokenAddress?.toLowerCase())
        .map(
          (token) =>
            new Token({
              chainId,
    ...
    [tokenRepresentations, chainId, bidTokenAddress],
  )
        ...
        ? liquidityPositions?.map((lp) => {
            ...
            return {
              ...
              balance: CurrencyAmount.fromRawAmount(
                lpToken,
                parseUnits(lp.liquidityTokenBalance, LP_DECIMALS).toString(),
              ),
              totalSupply: CurrencyAmount.fromRawAmount(
                lpToken,
                parseUnits(lp.pair.totalSupply, LP_DECIMALS).toString(),
              ),
            }
          })
        : [],
